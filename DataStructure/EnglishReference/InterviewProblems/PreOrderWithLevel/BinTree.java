...
public class BinTree<E> {
   ...
   void visit3(BinNode<E> rt, int level) {
     if (rt != null) {
         System.out.println(rt.key() + ", level = " + level);
     } else {
        System.out.println("The node is null");
     }
   }

   void preorder3(BinNode<E> rt, int level) {
      if (rt == null){
          return;
      }
      visit3(rt, level);
      if (rt.left()!= null) {
          preorder3(rt.left(), level+1);
      }
      if (rt.right()!= null) {
          preorder3(rt.right(), level+1);
      }
   }
  ...
  /** postorder tree walk: left, right, root */
   ...
   public static void main(String[] args) {
      ...
      //call preorder3 to print both the value and level number for each node
      bTree.preorder3(bTree.root(), 0);

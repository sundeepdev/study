/*
    Given a BST, how will you find median in that?

    Solution 1: Use inorder to add all n elements into an array A.

    if (A.length ^ 1 == 1) return A[A.length/2]
    else return (A[A.length/2-1] and A[A.length/2]) / 2
    ...
    I think it's possible to reduce the space complexity to O(1).

    1)  We use a counter(initialized as 0) to count how many nodes in total.
        We can use any type of traversal order to achieve this.

    2)  From step 1) we get the number of all nodes let's say it is called
        n. So now we use inorder traversal to the the median. i.e.

        For n = 10, since it is a even nuber we need to find two numbers
        which are [n/2-1, n/2 = [4, 5] in order to calculate the median.

        We first initialize a counter = 0. While doing inorder traversal, 
        we increment the counter whenever we see Node. When the counter
        hits 4 and 5, we can calculate the median.
  ...
  /** postorder tree walk: left, right, root */
   ...
   BinNode<E> delete(BinNode<E> z) {
    ...
    public void findMedian() {
        //First, we need to calculate how many nodes in total.
        n = count(root);
        count = 0;
    ...
    public void inorder3(BinNode<E> rt, int n) {
        if (rt == null) {
            return;
        }
        ...
        if(count == (n/2+1)) {
            median = rt.key();
            if (n % 2 == 0) {
                BinNode<E> predecessor = predecessor(rt);
                median = (predecessor.key() + median) / 2;
            }
        }
        if (rt.right() != null) {
            inorder3(rt.right(), n);
   ...
   public static void main(String[] args) {
      ...
      System.out.println(bTree.getMedian());
